  ...
  return (
    <>
      ...
      <aside className={clsx(
        'fixed inset-y-0 left-0 z-50 w-64 flex flex-col bg-[#001A34] border-r border-[#0D2D52] transition-transform lg:translate-x-0',
        open ? 'translate-x-0' : '-translate-x-full',
      )}>
        <div className="flex items-center justify-between px-5 py-5 border-b border-[#0D2D52]">
          <div>
            <p className="text-lg font-black text-white tracking-tight">MARKET</p>
            <p className="text-[10px] text-[#7A8FA6] uppercase tracking-widest">Admin Panel</p>
          </div>
            ...
            return (
              <Link key={href} href={href} onClick={() => setOpen(false)}
                className={clsx('flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors',
                  active ? 'bg-[#005BFF] text-white' : 'text-[#7A8FA6] hover:text-white hover:bg-[#0D2D52]')}>
                <Icon className="h-4 w-4 shrink-0" />{label}
              </Link>
            );
          })}
        </nav>
        <div className="p-4 border-t border-[#0D2D52] space-y-2">
